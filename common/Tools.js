...
class Tools {
	 ...
	 */
	getRect(selector, all, thiz) {
	  return new Promise((resolve) => {
	    uni.createSelectorQuery()
	      .in(thiz)[all ? 'selectAll' : 'select'](selector)
		...
		if (url.startsWith("tel:")) {
			...
			if (url.startsWith("http://") || url.startsWith("https://")) {
				...
				if(!getApp()){
					...
					uni.hideHomeButton()
					return
				}else if ((getApp().globalData && getApp().globalData.tabBar && getApp().globalData.tabBar.indexOf(url) != -1) || params['tabbar']) {
					uni.switchTab({
						url: url
					});
				}  else if (params && (params['redirect']||params['reLaunch'])) {
				    delete params['url']
