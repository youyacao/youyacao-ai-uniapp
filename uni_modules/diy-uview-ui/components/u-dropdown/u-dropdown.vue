<template>
	<view class="u-dropdown">
		<view class="u-dropdown__menu" :style="{
			...
			<view class="u-dropdown__menu__item" v-for="(item, index) in menuList" :key="index" @tap.stop="menuClick(index)">
				<view class="u-flex">
					...
					<view class="u-dropdown__menu__item__arrow" v-if="direction=='up'" :class="{
					...
					<view class="u-dropdown__menu__item__arrow" v-else :class="{
						'u-dropdown__menu__item__arrow--rotate': index === current
					}">
						<u-icon :custom-style="{display: 'flex'}" :name="menuIcon" :size="$u.addUnit(menuIconSize)" :color="index === current || highlightIndex == index ? activeColor : '#c0c4cc'"></u-icon>
					</view>
		...
		<view class="u-dropdown__content" :class="direction"  :style="[contentStyle, {
		 ...
		 @tap="maskClick" @touchmove.stop.prevent>
			...
			<view v-if="isMask" class="u-dropdown__content__mask"></view>
		</view>
	</view>
</template>
...
<script>
	 ...
	 * @property {String} active-color 标题和选项卡选中的颜色（默认#19be6b）
		...
		methods: {
			...
			getTopHeight(){
				...
				return new Promise(function(resolve, reject) {
					thiz.$uGetRect('#' + thiz.elid).then(res => {
					   if(res){
						   resolve(res)
					   }else{
						   resolve({top:0,height:40})
					   }
					})
				})
			},
			...
			getContentHeight() {
				...
				if(this.direction=='up'){
					...
					this.contentStyle.bottom =  bottom + 'px'
				}else{
					this.contentHeight = windowHeight  - this.menuPositionData.top;
					this.contentStyle.top =  this.topheight+'px'
				}
				/*this.$uGetRect('.u-dropdown__menu').then(res => {
					...
					this.contentHeight = windowHeight - res.bottom - this.topheight;
				})*/
			}
		}
	}
