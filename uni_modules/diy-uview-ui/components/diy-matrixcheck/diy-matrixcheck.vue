		...
		<view class="flex-sub flex-table flex  flex-direction-column"
			...
			<view class="flex items-stretch" v-for="(rowitem, rowindex) in rows">
				...
				<view class="td td1 flex  justify-center align-center" @click="change(rowitem[valueName],colitem[valueName])"
					...
					<text class="diy-icon-squarecheckfill" :style="getStyle(rowitem[valueName],colitem[valueName])"></text>
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			getStyle(row, col) {
				...
				if (rowItem && rowItem['cols'].includes(col)) {
			...
			change(row, col) {
				...
				if (index >= 0) {
					...
					if (rowItem.cols.includes(col)) {
						let cindex = rowItem.cols.findIndex(item => {
							return item == col
						})
						rowItem.cols.splice(cindex, 1)
						rowItem.cols.sort()
						if (rowItem.cols.length == 0) {
							values.splice(index, 1)
						}
					} else {
						rowItem.cols.push(col)
						rowItem.cols.sort()
					}
				} else {
					let rowItem = {
