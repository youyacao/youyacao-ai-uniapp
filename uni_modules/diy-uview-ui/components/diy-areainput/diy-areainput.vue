	...
	export default {
		...
		methods: {
			initDefaultValue(findValue) {
				if(findValue){
					let values = findValue.split("-")
					let data = []
					let changeCitys = {}
					let changeAreas = {}
					let provinceIndex = 0;
					if(values.length>=1&&values[0]){
						provinceIndex = provinces.findIndex((v, k) => {
							return v.code== values[0]
						});
						changeCitys = citys[provinceIndex]
						data.push(provinces[provinceIndex].name)
					}
					
				    let cityIndex = 0
					if(values.length>=2&&values[1]){
						 cityIndex = changeCitys.findIndex((v, k) => {
							return v.code== values[1]
						});
						changeAreas = areas[provinceIndex][cityIndex]
						data.push(changeCitys[cityIndex].name)
					}
					if(values.length>=3&&values[2]){
						let areaIndex = changeAreas.findIndex((v, k) => {
							return v.code== values[2]
						});
						data.push(changeAreas[areaIndex].name)
					}
