<template>
		<movable-area class="u-swipe-action" :style="{ backgroundColor: bgColor }">
			<movable-view class="u-swipe-view" @change="change" @touchend="touchend" @touchstart="touchstart"
				direction="horizontal" :disabled="disabled" :x="moveX" :style="{
					width: movableViewWidth ? movableViewWidth : '100%'
				}">
				<view class="u-swipe-content"  @tap.stop="contentClick">
					<slot></slot>
				</view>
				<view class="u-swipe-del" v-if="showBtn&&!$slots.right" @tap.stop="btnClick(index)"
					:style="[btnStyle(item.style)]" v-for="(item, index) in options" :key="index">
					<view class="u-btn-text">{{ item.text }}</view>
				</view>
				<view :style="buttonsWidthStyle" class="flex flex-nowrap" v-else>
					<slot name="right" :style="[btnStyle]"/>
				</view>
			</movable-view>
		</movable-area>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			movableViewWidth() {
				console.log(this.movableAreaWidth)
			...
			contentViewWidth() {
				...
				if (this.movableAreaWidth) {
					style = {
						width: this.movableAreaWidth + 'px'
					}
				} else {
					style = {
						flex: 1
			...
			buttonsWidthStyle(){
				let style = {
					paddingLeft:'1px',
					width:this.buttonsWidth + 'rpx'
				};
				return style;
			},
			allBtnWidth() {
				return this.buttonsWidth ? uni.upx2px(this.buttonsWidth): uni.upx2px(this.btnWidth) * this.options.length;
			},
			btnStyle() {
				return style => {
					let css = {};
