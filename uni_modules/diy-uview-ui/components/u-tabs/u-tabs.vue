<template>
	<view class="u-tabs" :style="{
		...
		<view :id="id">
			<scroll-view  :show-scrollbar="false" enhanced  scroll-x class="u-scroll-view" :scroll-left="scrollLeft" scroll-with-animation>
				<view class="u-scroll-box" :class="{'u-tabs-scorll-flex': !isScroll}">
					<view class="u-tab-item u-line-1" :id="'u-tab-item-' + index" v-for="(item, index) in list" :key="index" @tap="clickTab(index)"
					 :style="[tabItemStyle(index)]">
						<u-badge :bgColor="item['badgeBgColor']||'#ff0000'" :color="item['badgeColor']||'#fff'" :count="item[count] || item['count'] || 0" :offset="offset" size="mini"></u-badge>
						{{ item[name] || item['name']}}
					</view>
...
<script>
	 ...
	 * @property {String} active-color 滑块和激活tab文字的颜色（默认#19be6b）
