...
export default {
	...
	data() {
		return {
			...
			activeColor: "#19be6b", // 激活时左边文字和右边对勾图标的颜色
			...
			if(this.title){
				...
				if(this.options&&this.options.length>0){
					let find = this.options.find(item=>{
						return item.value == this.valueCom
					})
					if(find){
						return find.text||find.label
					}
				}
			}
		},
		...
		cellClick(value) {
			...
			this.$nextTick(()=>{
				if(!this.title){
					let parent = this.$u.$parent.call(this, "u-dropdown");
					if(parent){
						let index = parent.menuList.findIndex(item=>{
							return item.menuId == this.menuId
						})
						if(index>=0){
							parent.menuList[index].title = this.getDropTitle()
						}
					}
				}
			})
