	...
	export default {
		...
		methods: {
			...
			getColumnIndex(list,findValue,values,columnIndex){
				let index = 0;
				if(findValue[columnIndex]&&findValue.length>columnIndex){
				...
				if(this.isDefaultSelect){
					if(this.mode == 'single-column'){
						let index = this.list.findIndex(item=>{
							 return this.defaultSelectValue==item[this.valueName]
						})
						this.defaultSelector = [index]
					}else if(this.mode == 'mutil-column-auto'){
						let columnIndex = 0;
						let values = [] 
						this.getColumnIndex(this.list,this.defaultSelectValue,values,columnIndex)
					    this.defaultSelector = values
					}else{
						let defaultSelector = []
						this.list.forEach((column,index)=>{
							let findIndex = column.findIndex(item=>{
								 return this.defaultSelectValue.length>=index&&this.defaultSelectValue[index]==item[this.valueName]
							})
							if(findIndex<0){
								findIndex = 0
							}
