		...
		<view class="u-datetime-picker">
			...
			<view class="u-picker-body">
				...
				>
					...
					<picker-view-column v-if="!reset && params.day">
						<view class="u-column-item" v-for="(item, index) in penddays" :key="index">
							{{ formatNumber(item) }}
							<text class="u-text" v-if="showTimeTag">日</text>
						</view>
					</picker-view-column>
...
export default {
	...
	methods: {
		...
		initTimeValue(prefix='') {
			...
			// 获取年日月时分秒
			this[prefix+'year'] = time.getFullYear();
			this[prefix+'month'] = Number(time.getMonth()) + 1;
			this[prefix+'day'] = time.getDate();
			this[prefix+'hour'] = time.getHours();
			this[prefix+'minute'] = time.getMinutes();
			this[prefix+'second'] = time.getSeconds();
		},
		intValue(val){
			return parseInt(val)
		},
		initRangeTimeValue() {
			if(!this.defaultTime){
				this.initTimeValue('')
				this.initTimeValue('pend')
			}else{
				// 格式化时间，在IE浏览器(uni不存在此情况)，无法识别日期间的"-"间隔符号
				let values = this.defaultTime.split(" - ")
				if(this.mode=='yearRange'){
					this.year = this.intValue(values[0])
					this.pendyear = this.intValue(values[1])
				}else if(this.mode=='monthRange'){
					this.month = this.intValue(values[0])
					this.pendmonth = this.intValue(values[1])
				}else if(this.mode=='hourRange'){
					this.hour = this.intValue(values[0])
					this.pendhour = this.intValue(values[1])
				}else if(this.mode=='yearMonthRange'){
					let start = values[0]
					let pend = values[1]
					let yearMonth = start.split("-")
					this.year = this.intValue(yearMonth[0])
					this.month = this.intValue(yearMonth[1])
					
					let pendYearMonth = pend.split("-")
					this.pendyear = this.intValue(pendYearMonth[0])
					this.pendmonth = this.intValue(pendYearMonth[1])
				}else if(this.mode=='yearMonthDateRange'){
					let start = values[0]
					let pend = values[1]
					let yearMonth = start.split("-")
					this.year = this.intValue(yearMonth[0])
					this.month = this.intValue(yearMonth[1])
					this.day = this.intValue(yearMonth[2])
					let pendYearMonth = pend.split("-")
					this.pendyear = this.intValue(pendYearMonth[0])
					this.pendmonth = this.intValue(pendYearMonth[1])
					this.pday  = this.intValue(pendYearMonth[2])
					
				}else if(this.mode=='hourMinuteRange'){
					let start = values[0]
					let pend = values[1]
					let hourMinute = start.split(":")
					this.hour = this.intValue(hourMinute[0])
					this.minute = this.intValue(hourMinute[1])
					
					let pendHourMinute = pend.split(":")
					this.pendhour = this.intValue(pendHourMinute[0])
					this.pendminute = this.intValue(pendHourMinute[1])
				}
			}
		},
		init() {
			...
			if (this.mode == "time") {
			...
			}else if (this.mode.endsWith("Range")) {
				this.initRangeTimeValue();
				if (this.params.year) {
					this.valueArr.push(0);
					this.setYears('');
				}
				...
				if (this.params.minute) {
					this.valueArr.push(0);
					this.setMinutes('pend');
				}
				if (this.params.second) {
					this.valueArr.push(0);
					this.setSeconds('pend');
				}
			} else if (this.mode == "region") {
				if(this.areaCodes){
					this.searchAreaCode = this.areaCodes.split("-")
				}else{
					this.searchAreaCode = this.areaCode
				}
		...
		setYears(prefix='') {
			...
			this.valueArr.splice(this.valueArr.length - 1, 1, this.getIndex(this.years, this[prefix+'year']));
		},
		setMonths(prefix='') {
			this.months = this.generateArray(1, 12);
			this.valueArr.splice(this.valueArr.length - 1, 1, this.getIndex(this.months, this[prefix+'month']));
		},
		setDays() {
		...
		setEndDays() {
			let totalDays = new Date(this.pendyear,this.pendmonth, 0).getDate();
			this.penddays = this.generateArray(1, totalDays);
			let index = 0;
			// 这里不能使用类似setMonths()中的this.valueArr.splice(this.valueArr.length - 1, xxx)做法
			// 因为this.month和this.year变化时，会触发watch中的this.setDays()，导致this.valueArr.length计算有误
			if (this.params.year && this.params.month) index = 5;
			else if (this.params.month) index = 4;
			else if (this.params.year) index = 4;
			else index = 3;
			// 当月份变化时，会导致日期的天数也会变化，如果原来选的天数大于变化后的天数，则重置为变化后的最大值
			// 比如原来选中3月31日，调整为2月后，日期变为最大29，这时如果day值继续为31显然不合理，于是将其置为29(picker-column从1开始)
			if (this.pendday > this.penddays.length) this.pendday = this.penddays.length;
			this.valueArr.splice(index, 1, this.getIndex(this.penddays, this.pendday));
		},
		setHours(prefix='') {
			this.hours = this.generateArray(0, 23);
			this.valueArr.splice(this.valueArr.length - 1, 1, this.getIndex(this.hours, this[prefix+'hour']));
		},
		setMinutes(prefix='') {
			this.minutes = this.generateArray(0, 59);
			this.valueArr.splice(this.valueArr.length - 1, 1, this.getIndex(this.minutes, this[prefix+'minute']));
		},
		setSeconds(prefix='') {
			this.seconds = this.generateArray(0, 59);
			this.valueArr.splice(this.valueArr.length - 1, 1, this.getIndex(this.seconds, this[prefix+'second']));
		},
		setProvinces() {
			// 判断是否需要province参数
			...
			if (this.searchAreaCode.length) {
				tmp = this.searchAreaCode[0];
				useCode = true;
		...
		change(e) {
			...
			if (this.mode == "time") {
				...
				this.rangeValue = this.getRangeValue()
			}else if (this.mode == "multiSelector") {
				let index = null;
		...
		getTimeValue(i){
			let result = {}
			if (this.params.year) result.year = this.formatNumber(this.years[this.valueArr[i++]]|| 0);
			if (this.params.month) result.month = this.formatNumber(this.months[this.valueArr[i++]]|| 0);
			if (this.params.day) result.day = this.formatNumber(this.days[this.valueArr[i++]]|| 0);
			if (this.params.hour) result.hour = this.formatNumber(this.hours[this.valueArr[i++]]|| 0);
			if (this.params.minute) result.minute = this.formatNumber(this.minutes[this.valueArr[i++]]|| 0);
			if (this.params.second) result.second = this.formatNumber(this.seconds[this.valueArr[i++]]|| 0);
			return [result,i];
		},
		createDate(year, month, day) {
		    // 注意：JavaScript 中的月份是从 0 开始的（0 = 一月, 11 = 十二月）
		...
		getRangeValue(){
			...
			// 这里使用i++，是因为this.valueArr数组的长度是不确定长度的，它根据this.params的值来配置长度
			// 进入if规则，i会加1，保证了能获取准确的值
			let startValue = this.getTimeValue(i);
			let start  = startValue[0];
			let endValue = this.getTimeValue(startValue[1]);
			let end  = endValue[0];
			let rangeValue = ""
			if(this.mode=='yearMonthRange'){
				...
				if((startYear>endYear)||(startYear<=endYear&&startMonth>endMonth)){
					uni.$u.toast('开始年月不能大于结束年月')
					return;
				}
				rangeValue = startYear+"-"+startMonth+" - "+ endYear+"-"+endMonth
			}else if(this.mode=='yearMonthDateRange'){
				let startYear = start.year;
				let endYear = end.year;
				...
				let startDate = this.createDate(startYear,startMonth,startDay)
				let endDate = this.createDate(endYear,endMonth,endDay)
				if(startDate>endDate){
					uni.$u.toast('开始日期不能结束日期')
					return;
				}
				rangeValue = startYear+"-"+startMonth+"-"+startDay+" - "+ endYear+"-"+endMonth+"-"+endDay
			}else if(this.mode=='hourMinuteRange'){
				...
				rangeValue = startHour+":"+startMinute+" - "+ endHour+":"+endMinute
			}else if(this.mode=='hourRange'){
				let startHour = start.hour;
				let endHour = end.hour;
				...
				rangeValue  = startHour+" - "+endHour
			}else if(this.mode=='yearRange'){
				let startYear = start.year;
				let endYear = end.year;
				if(startYear>endYear){
					uni.$u.toast('开始年不能大于结束年')
					return;
				}
				...
				if(startMonth>endMonth){
					uni.$u.toast('开始月不能大于结束月')
					return;
				}
				rangeValue  = startMonth+" - "+endMonth
			}
			return rangeValue;
		},
		// 用户点击确定按钮
